...
class Payment
{
    ...
    {
        $this->corePayment = new \Ipol\Demo\Core\Order\Payment();
    ...
    public function fromOrder($bitrixId)
    {
        ...
        $this->getCorePayment()->setDelivery($obMoneyDelivery)
            ...
            ->setEstimated(($this->options->fetchNoInsurance() === 'Y') ? new Money(0) : $obMoneyEstimated);

        $nOrder    = \Ipol\Demo\Bitrix\Handler\Order::getOrderById($bitrixId);
        $payNal    = false;
        $payCard   = false;
        $allPayed  = false;
...
//        $this->getCorePayment()->setNdsDelivery($this->options->getNdsDelivery()); // TODO: checkNDSPelivery
    ...
    /**
     * @return \Ipol\Demo\Core\Order\Payment
     */
