...
use Ipol\Demo\OrderPropsHandler;
use Ipol\Demo\OrdersTable;

class Order
{
    /**
     * @var string if constant defined, bitrix order ID used as barcode instead of barkCounter option
     */
    protected static $idAsBarcode = 'IPOL_DEMO_IDASBARCODE';
    ...
    {
        $this->moduleLbl   = IPOL_DEMO_LBL;
        $this->options     = $options;
        $this->baseOrder   = new \Ipol\Demo\Core\Order\Order();
    ...
    public function newOrder($bitrixId)
    {
        ...
        $this->orderNumber = \Ipol\Demo\Bitrix\Handler\Order::getOrderNumber($bitrixId);
        ...
        $order = \Ipol\Demo\Bitrix\Handler\Order::getOrderById($this->bitrixId);
        if ($order) {
    ...
    protected function setDefaultFields()
    {
        ...
        $uniqueItemCode = (defined(self::$idAsBarcode) && constant(self::$idAsBarcode) === true) ? $this->bitrixId : null;
        ...
        $this->getBaseOrder()
            ...
            ->setField('senderCreateDate', \Ipol\Demo\Bitrix\Handler\Order::getOrderDate($this->bitrixId, true))
            ->setField('barcode', $barcodeGenerateByServer ? null : OrderHandler::getOrderBarcode($uniqueItemCode))
    ...
    public function requestOrder()
    {
        ...
        $arDateCreate  = \Ipol\Demo\Bitrix\Handler\Order::getOrderDate($this->bitrixId, true);
        $this->getBaseOrder()->setField('createDate', $arDateCreate['timestamp']);
        $this->getBaseOrder()->setField('createDateFP', $this::makeDemoTimeFromTimestamp($arDateCreate['timestamp']));
        ...
        if (array_key_exists('plannedReceiveDate', $_REQUEST) && $_REQUEST['plannedReceiveDate']) {
            ...
            $this->getBaseOrder()->setField('plannedReceiveDateFP', $this::makeDemoTimeFromTimestamp($_REQUEST['plannedReceiveDate']));
        ...
        if (array_key_exists('shipmentDate', $_REQUEST) && $_REQUEST['shipmentDate']){
            ...
            $this->getBaseOrder()->setField('shipmentDateFP', $this::makeDemoTimeFromTimestamp($_REQUEST['shipmentDate']));
    ...
    protected static function fromRequest()
    {
        return array(
            ...
            'payment'     => array(
                'goods'      => $_REQUEST['goodsPrice'],
                'delivery'   => $_REQUEST['deliveryPrice'],
                'estimated'  => $_REQUEST['estimatedCost'],
            ...
            'order' => array(
                ...
                'prepaymentCurrency'      => $_REQUEST['prepaymentCurrency'],
    ...
    public function uploadedOrder($bitrixId)
    {
        ...
        if ($arOrder) {
            ...
            $this->getBaseOrder()->setLink($arOrder['DEMO_GUID']);
            ...
            $this->getBaseOrder()
                ->setNumber(($this->getOrderNumber()) ? $this->getOrderNumber() : \Ipol\Demo\Bitrix\Handler\Order::getOrderNumber($bitrixId))
                ->addReciever($this->getReceiver()->getCoreReceiver())
                ->setAddressTo($this->getAddressTo()->getCoreAddress())
                ->setPayment($this->getPayment()->getCorePayment())
                ->setGoods($dbFields['goods'])
                ->setItems($dbFields['items'])
                ->setField('createDate', $this::makeDemoTimeFromTimestamp($arDateCreate['timestamp']))
    ...
    protected function fromDB($dataRow)
    {
        ...
        return array(
            ...
            'order' => array(
                ...
                'barcode'                   => ($dataRow['DEMO_ID'] ? $dataRow['DEMO_ID'] : null),
                'barcodeGenerateByServer'   => ($dataRow['BARK_GENERATE_BY_SERVER'] === 'Y'),

                'demo_status'           => $dataRow['DEMO_STATUS'],
                'demo_execution_status' => $dataRow['DEMO_EXECUTION_STATUS'],
                ...
                'prepaymentCurrency'        => $dataRow['PREPAYMENT_CURRENCY'],
    ...
    {
        $obOrder = OrdersTable::getByDemoId($id);
        if ($obOrder) {
            return $this->uploadedOrder($obOrder['BITRIX_ID']);
        } else {
            throw new \Exception('No order with demo ID '.$id);
        }
    }
    ...
    protected static function makeDemoTimeFromTimestamp($timeStamp)
    {
