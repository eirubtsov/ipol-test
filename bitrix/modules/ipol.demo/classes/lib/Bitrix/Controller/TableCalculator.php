...
use \Ipol\Demo\RatesTable;

use \Ipol\Demo\Bitrix\Adapter;
use \Ipol\Demo\Core\Delivery\Shipment;
use \Ipol\Demo\Core\Delivery\Tariff;
use \Ipol\Demo\Core\Delivery\Tools;
...
{
    /**
     * @var \Ipol\Demo\Core\Delivery\Shipment
     */
            ...
            if (!empty($points))
            {
                ...
                if (!empty($rates))
                {
                    ...
                    foreach ($rates as $zoneId => $zone)
                    {
                        ...
                        {
                            // Checks if rate values are useful. Otherwise client must ask demo manager about delivery rates
                    ...
                    {
                        $result->addError(new Error('No loaded rates, all rate values are zero. Contact your demo manager about creating rate data for you'));
                    }
     ...
     *
     * @param array $filter @see Ipol\Demo\Bitrix\Controller\TableCalculator::preparePointFilter()
     * @return array
     */
    protected function getPossiblePoints($filter)
    {
        ...
        $cacheTime     = 86400;
        $cachePath     = '/'.IPOL_DEMO.'/getPossiblePoints';
     ...
     *
     * @param array $filter @see Ipol\Demo\Bitrix\Controller\TableCalculator::prepareRatesFilter()
     * @return array
     */
    protected function getPossibleRates($filter)
    {
        ...
        $cacheTime     = 86400;
        $cachePath     = '/'.IPOL_DEMO.'/getPossibleRates';
        $cacheInstance = \Bitrix\Main\Data\Cache::createInstance();
        if ($cacheInstance->initCache($cacheTime, $cacheId, $cachePath))
        {
