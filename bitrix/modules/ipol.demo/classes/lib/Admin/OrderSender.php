...
class OrderSender
{
    ...
    public static function init()
    {
        ...
        if (
            ...
            // B24 support
            self::$workMode = 'order';
            $tmp = explode('/', ltrim($dir, $b24path['ORDER']));
            self::$orderId  = array_shift($tmp);
            self::$workType = 'b24';
        }
        ...
        if (Option::get('showInOrders') === 'Y' || Deliveries::isdemoDelivery(self::getId())) {
    ...
    protected static function getOrderExportJs(\Ipol\Demo\Core\Order\Order $order)
    {
        ...
        <script type="text/javascript">
                var <?=self::$MODULE_LBL?>export = new idemo_adminInterface({
            ...
            <?=self::$MODULE_LBL?>export.expander({
                ...
                demoId    : '<?=$order->getLink()?>',
                wayBill       : '<?=$order->getField('waybill')?>',
                workMode      : '<?=self::$workMode?>',
                status        : '<?=self::$status?>',
                goodsPrice    : '<?=$order->getPayment()->getGoods()->getAmount()?>',
                estimatedCost : '<?=$order->getPayment()->getEstimated()->getAmount()?>',
        ...
        <?php include_once('OrderSenderPages/main.php')?>
        <?php include_once('OrderSenderPages/gabs.php')?>
        <?php include_once('OrderSenderPages/goods.php')?>
    ...
    public static function generateExportOrderHtml(\Ipol\Demo\Core\Order\Order $order)
    {
        ...
        <div id="<?=self::$MODULE_LBL?>PLACEFORFORM">
            <table id="<?=self::$MODULE_LBL?>wndOrder">
                ...
                <?php if($order->getField('message')){?><tr><td colspan="2" class="<?=self::$MODULE_LBL?>warning"><?=$order->getField('message')?></td></tr><?php }?>
                <?php if($order->getLink()){?>
                    <tr><td><?=Tools::getMessage('LBL_DEMO_GUID')?></td><td><?=$order->getLink()?></td></tr>
                <?php }?>
                <?php if($order->getField('cargoesCargoId')){?>
                    <?php Tools::placeSOHeaderRow('CARGOES',self::$MODULE_LBL."export.getPage('main').ui.toggleBlock('cargoes')");?>
                ...
                <?php }?>
                <?php Tools::placeSOHeaderRow('COMMONDATA')?>
                <?php Tools::placeSORow('number','hidden',$order->getNumber());?>
                <?php $sd = $order->getField('senderCreateDate');
                Tools::placeSORow('senderCreateDate','sign',$sd['sign']);?>
                <tr><td>
                    <input type="hidden" value="<?=$sd['timestamp']?>" id="<?=self::$MODULE_LBL?>senderCreateDate">
                </td></tr>
                <?php if ($order->getField('barcode')) {?>
                    <?php Tools::placeSORow('barcode','hidden',$order->getField('barcode'));?>
                <?php }?>
                ...
                <?php Tools::placeSORow('barcodeGenerateByServer','hidden',$order->getField('barcodeGenerateByServer'),false,false,"unseen");?>
                <tr><td colspan="2"><hr></td></tr>
                <tr>
                    <td><?=Tools::getMessage('LBL_plannedReceiveDate')?></td>
                    <td><?php if ($order->getField('plannedReceiveDate')) {
                ...
                <?php //GOODS_SPECIAL?>
                <?php Tools::placeSOHeaderRow('GOODS_SPECIAL',self::$MODULE_LBL."export.getPage('main').ui.toggleBlock('goodsSpecial')");?>
                <?php Tools::placeSORow('brandName','text',$order->getField('brandName'),false,false,'block_goodsSpecial');?>
                <?php Tools::placeSORow('undeliverableOption','select',$order->getField('undeliverableOption'),Adapter::getUO(),false,'block_goodsSpecial');?>
                <tr>
                    <td><?=Tools::getMessage('LBL_shipmentDate')?></td>
                    <td><?php if ($order->getField('shipmentDate')) {
                ...
                <?php //Tools::placeSORow('senderCargoId','text',false,false,false,'block_goodsSpecial');?>

                <?php //GABARITES?>
                <?php Tools::placeSOHeaderRow('GABARITES',self::$MODULE_LBL."export.getPage('main').ui.toggleBlock('gabariles')");?>
                ...
                <?php // RECEIVER?>
                <?php $receiver = $order->getReceivers()->getFirst();?>
                <?php Tools::placeSOHeaderRow('RECEIVER');?>

                <?php Tools::placeSORow('clientName','text',$receiver->getFullName());?>
                <?php Tools::placeSORow('clientEmail','text',$receiver->getEmail());?>
                <?php Tools::placeSORow('clientPhone','text',$receiver->getPhone());?>
                <?php // PICKUPS?>
                ...
                <?php // PAYMENT?>
                <?php Tools::placeSOHeaderRow('PAYMENT')?>
                <?php Tools::placeSORow('estimatedCost','hidden',$order->getPayment()->getCost()->getAmount());?>
                <tr><td></td><td><small><?=Tools::getMessage('LBL_INSURANCE')?></small></td></tr>
                <tr><td colspan="2"><hr></td></tr>
                <tr><td colspan="2">
                    <?php // TODO: refactor?>
                    ...
                    <input type="hidden" value="RUB" id="<?=self::$MODULE_LBL?>prepaymentCurrency">
                    <input type="hidden" value="RUB" id="<?=self::$MODULE_LBL?>priceCurrency">
                </td></tr>
                <?php Tools::placeSORow('payment_isBeznal','checkbox',($order->getPayment()->getIsBeznal()),false,"onchange=\"".self::$MODULE_LBL."export.getPage('main').events.onPayedChange()\"");?>
                <?php Tools::placeSORow('deliveryPrice','text',$order->getPayment()->getDelivery()->getAmount(),false,"onchange=\"".self::$MODULE_LBL."export.getPage('main').events.onDeliveryPriceChange()\"");?>
                <?php Tools::placeSORow('paymentType','select',Adapter::convertPaymentTypes($order->getPayment()->getType()),Adapter::getPaymentTypes());?>
                <?php //Tools::placeSORow('payment_ndsDefault','select',$order->getPayment()->getNdsDefault(),Adapter::getNDSTypes());?>
                <?php //Tools::placeSORow('payment_ndsDelivery','select',$order->getPayment()->getNdsDelivery(),Adapter::getNDSTypes());?>
                <tr><td colspan="2"><hr></td></tr>
                <?php Tools::placeSORow('goodsPrice','hidden',$order->getPayment()->getGoods()->getAmount());?>
                <?php Tools::placeSORow('payed','hidden',$order->getPayment()->getPayed()->getAmount());?>
                <tr><td colspan="2"><hr></td></tr>
                <?php Tools::placeSORow('paymentTotal','hidden',($order->getPayment()->getIsBeznal() ? 0 : $order->getPayment()->getNominalPrice()->getAmount()));?>
                </tbody></table>
        </div>
        <?php
    }
    ...
    protected static function loadExportCSS(){
        ...
        <style>
            ...
            #<?=self::$MODULE_LBL?>goodsEdit input[type='text']{
                width: 120px;
            }

            #<?=self::$MODULE_LBL?>goodsEdit input[type='number']{
                width: 100px;
            }
