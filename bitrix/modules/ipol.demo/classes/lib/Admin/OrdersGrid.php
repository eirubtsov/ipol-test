...
use \Ipol\Demo\Admin\Grid\DatabaseGrid;
...
class OrdersGrid extends DatabaseGrid
{
    ...
    protected $defaultButtons = [
        [
            ...
            'TYPE'    => 'button',
            'ONCLICK' => IPOL_DEMO_LBL.'controller.getPage("main").actions.suncStatuses(this)',
        ],
    ];
    ...
    protected $defaultColumns = [
        ...
        [
            'id'          => 'PREPAYMENT_CURRENCY',
            'name'        => 'TABLE_ORDERS_PREPAYMENT_CURRENCY',
            'sort'        => 'PREPAYMENT_CURRENCY',
            'default'     => false,
            'editable'    => false,
            'filterable'  => false,
        ],
        ...
        [
            'id'          => 'DEMO_EXECUTION_STATUS',
            'name'        => 'TABLE_ORDERS_DEMO_EXECUTION_STATUS',
            'sort'        => 'DEMO_EXECUTION_STATUS',
            'default'     => true,
            'editable'    => false,
            'filterable'  => false,
        ],
    ...
    protected $defaultRowActions = [
        ...
        'GET_ORDER_STATUS' => [
            ...
            'ONCLICK'   => IPOL_DEMO_LBL.'controller.getPage("main").actions.suncOrderStatus("#BITRIX_ID#")',
        ],
        'GET_ORDER_STICKER' => [
            'ICONCLASS' => 'menu-popup-item-copy',
            'TEXT'      => 'TABLE_ORDERS_ROW_PRINT_STICKER',
            'ONCLICK'   => IPOL_DEMO_LBL.'controller.getPage("main").actions.getSticker("#BITRIX_ID#", #BARK_GENERATE_BY_SERVER#)',
        ],
        'ERASE_ORDER' => [
            'ICONCLASS' => 'menu-popup-item-delete',
            'TEXT'      => 'TABLE_ORDERS_ROW_ERASE_ORDER',
            'ONCLICK'   => IPOL_DEMO_LBL.'controller.getPage("main").actions.eraseOrder("#BITRIX_ID#")',
        ],
        'DELETE_ORDER' => [
            'ICONCLASS' => 'menu-popup-item-delete',
            'TEXT'      => 'TABLE_ORDERS_ROW_DELETE_ORDER',
            'ONCLICK'   => IPOL_DEMO_LBL.'controller.getPage("main").actions.cancelOrder("#BITRIX_ID#")',
        ],
    ];
                        ...
                        'DATA'   => [
                            [
                                'JS' => '
                                    ...
                                    if (ids.length > 0) {
                                        '.IPOL_DEMO_LBL.'controller.getPage("main").actions.printStickers(ids);
                                    }
                                ',
                            ]
                        ]
    ...
    protected function getRow($item)
    {
        ...
        // - drop .main-grid-cell background color required, check ipol_demo_orders.php
        $statusToColor = array(
            'ok'          => '#E2FCE2',
            'sended'      => '#E2FCE2',
            'valid'       => '#E2FCE2',
    ...
    protected function getRowActions($item)
    {
        ...
        foreach ($ret as $index => $action) {
            ...
            $ret[$index]['LINK']    = str_replace(['#BITRIX_ID#', '#DEMO_ID#', '#BARK_GENERATE_BY_SERVER#'], [$item['BITRIX_ID'], $item['DEMO_ID'], $genByServer], $action['LINK']);
            $ret[$index]['ONCLICK'] = str_replace(['#BITRIX_ID#', '#DEMO_ID#', '#BARK_GENERATE_BY_SERVER#'], [$item['BITRIX_ID'], $item['DEMO_ID'], $genByServer], $action['ONCLICK']);
        }
