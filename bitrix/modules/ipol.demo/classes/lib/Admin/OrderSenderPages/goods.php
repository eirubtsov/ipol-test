...
<script type="text/javascript">
    <?=self::$MODULE_LBL?>export.addPage('goods', {
        ...
        open: function () {
            ...
            <?php if(Adapter::statusIsSending(self::$status)){?>
        ...
        load: function () {
            this.wnd = new idemo_wndController({
        ...
        html: function () {
            ...
            container.html('');
            container.append('<tr class="<?=self::$MODULE_LBL?>addHeader"><th><?=Tools::getMessage('LBL_GOOD')?></th><th><?=Tools::getMessage('LBL_BARCODE')?></th><th><?=Tools::getMessage('LBL_VENDORCODE')?></th><th><?=Tools::getMessage('LBL_COST')?></th><th><?=Tools::getMessage('LBL_PRICE')?></th><th><?=Tools::getMessage('LBL_QUAN')?></th><th><?=Tools::getMessage('LBL_NDS')?></th><th><?=Tools::getMessage('LBL_ADDITIONAL')?></th></tr><tr><td><div id="<?=self::$MODULE_LBL?>QRSelector" class="b-popup" style="display: none;"><div class="pop-text"></div><div class="close" onclick="$(this).closest(\'.b-popup\').hide();"></div></div></td></tr>');
        ...
        gtd : {
            ...
            findItemInSaved : function (item,arSaved) {
                ...
                if(typeof(arSaved) === 'object'){
                    for (var number in arSaved) {
                        if (arSaved[number].id === item) {
                            found = arSaved[number];
                            break;
                        }
                    }
                }
...
//                    cis     : '',
        ...
        preSave: function () {
            var obSaves = ['name','articul','barcode','weight','price','cost','quantity','oc','ccd','tnved','id','vatRate'];
            var save    = {};
            this.self.items.forEach(function (item) {
                var obItem = {};
                obSaves.forEach(function (field) {
                    ...
                    if(input.length) {
                        if (field === 'cost') {
                            obItem[field] = parseFloat($('[_data_iid="' + item.id + '"].' + field).val().replace(',', '.'));
                        } else {
        ...
        checkSave : function(){
            var obReturn = {success: true, reason: false, estimatedCost: 0};
            var errorString = '';
            for (var i in this.info) {
                var itemLbl = this.info[i].name + ' [' + i + ']';
                if (isNaN(this.info[i].cost)) {
                    errorString += '<?=Tools::getMessage('LBL_GOOD')?> ' + itemLbl + " <?=Tools::getMessage('LBL_GOOD_ERROR_NOCOST')?>\n";
                    obReturn.success = false;
                } else {
                    obReturn.estimatedCost += this.info[i].cost * parseFloat(this.info[i].quantity);
                }
            }

            if (!obReturn.success) {
                obReturn.reason = errorString;
                obReturn.estimatedCost = -1;
            }
...
// buttons
        submit: function () {
            ...
            var closeBtn = $('#<?=self::$MODULE_LBL?>goodsEdit').closest('.bx-core-adm-dialog').find('.bx-core-adm-icon-close');
            var check = this.checkSave();
            if (check.success) {
                closeBtn.css('visibility', 'visible');
                this.wnd.close();
                <?=self::$MODULE_LBL?>export.getPage('main').events.onEstimatedCostChange(check.estimatedCost);
            } else {
