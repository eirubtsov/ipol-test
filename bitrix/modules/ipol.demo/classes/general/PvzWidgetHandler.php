...
class PvzWidgetHandler extends abstractGeneral
{
    ...
    public static function loadWidjet()
    {
        if (Deliveries::isActive() && $_REQUEST['is_ajax_post'] != 'Y' && $_REQUEST["AJAX_CALL"] != 'Y' && !$_REQUEST["ORDER_AJAX"]) {
            ...
            if (file_exists($_SERVER['DOCUMENT_ROOT'].$pathToWidjet) && file_exists($_SERVER['DOCUMENT_ROOT'].$pathToController)) {
                $GLOBALS['APPLICATION']->AddHeadScript($pathToWidjet);
                $GLOBALS['APPLICATION']->AddHeadScript($pathToController);
                ?>
                <script type="text/javascript">
                    var <?=self::getMODULELBL()?>PVZWidjet = new ipol_demo_pvzWidjet(
                        '<?=self::$city?>',
                        ...
                        <?=(Option::get('widgetShowListPvz') === 'Y') ? 'true' : 'false'?>,
...
//        return self::$MODULE_LBL.'pickup_pvz';
            ...
            if (($_REQUEST['is_ajax_post'] == 'Y' || $_REQUEST["AJAX_CALL"] == 'Y' || $_REQUEST["ORDER_AJAX"]) && $noJson) {
                $content
                    .= '<input type="hidden"
                        ...
                        'city'   => self::$city,
                        'paysys' => CargoHandler::definePaysystem(),
    ...
    /**
     * Returns city PVZ JSON
     * @param array $params
     * @return void
     */
    public static function getCityPVZ($params)
    {
        $result = ['P' => [], 'C' => []];

        $cargo = false;
        if (isset($params['cargo']) && is_array($params['cargo']) && isset($params['cargo'][0]) && is_array($params['cargo'][0])) {
            $cargo = new CargoItem();
            $cargo
                ->setPrice(new Money($params['cargo'][0]['price']))
                ->setQuantity(1)
                ->setWeight(floatval($params['cargo'][0]['weight']))
                ->setLength($params['cargo'][0]['length'])
                ->setWidth($params['cargo'][0]['width'])
                ->setHeight($params['cargo'][0]['height']);
        }

        $city = LocationsDelivery::getByFiasGuid($params['cityId']);

        if (!empty($city['BITRIX_CODE'])) {
            $pointsResult = PointsHandler::getPoints($cargo, false, $city['BITRIX_CODE']);
            if ($pointsResult->isSuccess()) {
                $points = $pointsResult->getData()['POINTS'];

                $result['C'] = [
                    'C' => $city['LOCALITY_CODE'], // CODE
                    'N' => $city['NAME'],          // NAME
                    'R' => $city['REGION'],        // REGION
                    'F' => $city['COUNTRY'].' '.$city['REGION'].' '.$city['NAME'],               // FULLREGION
                    'A' => implode(', ', array_filter(array($city['REGION'], $city['COUNTRY']))) // ALLREGIONINFO
                ];
                ...
                foreach ($points as $point) {
                    $pointsData[] = [
                        ...
                        'D' => $point['ADDITIONAL'],
                        'E' => $point['WORK_HOURS'],
                        'F' => $point['FULL_ADDRESS'],
                        'G' => $point['ADDRESS_LAT'],
                        'H' => $point['ADDRESS_LNG'],
                        // 'I' => $point['ADDRESS_METRO_STATION'],
                        // 'J' => $point['LOCALITY_FIAS_CODE'],
                        // 'K' => $point['MAX_CELL_WEIGHT'],
                        // 'L' => $point['MAX_CELL_DIMENSIONS_HASH'],
                        // 'M' => $point['PHONE'],
                        'N' => $point['CASH_ALLOWED'],
                        'O' => $point['CARD_ALLOWED'],
                        // 'P' => $point['LOYALTY_ALLOWED'],
                        // 'Q' => $point['DELIVERY_SL']
                    ];
                }
                $result['P'] = $pointsData;
            } else {
                $result['error'] = implode(', ', $pointsResult->getErrorMessages());
            }
        }

        self::toAnswer(array('pvz' => $result));
        self::printAnswer();
    }
    ...
    public static function getLangArray()
    {
        $tanslate = array(
            'rus' => array(
                ...
                'ERROR_ZERO_PVZ' => Tools::getMessage('WIDJET_ERROR_ZERO_PVZ'),
            )
        );
    ...
    public static function calcPVZ(){
        ...
        if(!$blockedByFilter) {
            ...
            if ($obCity) {
                ...
                $arCalc  = CargoHandler::calculateDelivery($arOrder,$setter);
            }
        }
