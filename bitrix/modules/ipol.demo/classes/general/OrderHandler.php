...
use Ipol\Demo\Bitrix\Adapter\Order;
...
{
    /**
     * @var \Ipol\Demo\Bitrix\Adapter\Order
     */
    public static $order;

    /**
     * @return \Ipol\Demo\Bitrix\Adapter\Order
     */
    ...
    public static function loadUploadOrderBy5I($DEMOId)
    {
        self::initOrder();
        return self::getOrder()->uploadedOrderBy5I($DEMOId)->getBaseOrder();
    }
    ...
    public static function sendOrder()
    {
        ...
        if ($resultAdd[0]->isSuccess()) {
            ...
            $controller = new \Ipol\Demo\Bitrix\Controller\Order(self::$MODULE_ID, self::$MODULE_LBL, $obOrder);
            $sendResult = $controller->send();
            if ($sendResult->isSuccess()) {
                /** @var \Ipol\Demo\Api\Entity\Response\OrdersMake $response */
                ...
                if ($contents->getQuantity()) {
                    ...
                    while ($content = $contents->getNext()) {
                        if ($content->isCreated()) {
                            ...
                            $cargoData = [
                                'cargoId'       => '', // demo cargo UUID
                                'senderCargoId' => '',
                                'barcode'       => '', // demo unique cargo barcode
                            ];
                            ...
                            $result->setSuccess(true)->setData(['DEMO_ORDER_UUID' => $orderUuid]);

                            self::markOrderSended($obOrder->getField('orderId'), $orderUuid, $cargoData);

                            if (Options::fetchOption('addTracking') == 'Y') {
                                \Ipol\Demo\Bitrix\Handler\Order::addTracking($obOrder->getField('orderId'), $obOrder->getField('track'));
                            }
                        } else {
                            // Some errors raised
        ...
        if (Tools::isModuleAjaxRequest()) {
            echo Tools::jsonEncode(array(
                ...
                'demoUuid' => $result->isSuccess() ? $result->getData()['DEMO_ORDER_UUID'] : false
            ));
        }
    ...
    public static function getOrderBarcode($uniqueItemCode)
    {
        $controller = new \Ipol\Demo\Bitrix\Controller\Order(self::$MODULE_ID, self::$MODULE_LBL);

        if (Tools::isModuleAjaxRequest() && $_REQUEST[self::$MODULE_LBL.'action'] === __METHOD__) {
            echo $controller->generateBarcode($uniqueItemCode);
        } else {
            return $controller->generateBarcode($uniqueItemCode);
        }
    ...
    /**
     * Save order to DB
     * @param \Ipol\Demo\Core\Order\Order $obOrder
    ...
    public static function saveOrder($obOrder)
    {
        ...
        $data = array(
            ...
            'DEMO_ID'               => $obOrder->getField('barcode'),
            'DEMO_GUID'             => '',
            ...
            'PREPAYMENT'                => $obOrder->getPayment()->getPayed()->getAmount(),
            'PREPAYMENT_CURRENCY'       => $obOrder->getField('prepaymentCurrency'),
     ...
     * @param int $bitrixId
     * @param string $demoUuid
     * @param array $cargoData
     */
    public static function markOrderSended($bitrixId, $demoUuid, $cargoData)
    {
