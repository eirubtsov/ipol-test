...
use Ipol\Demo\Bitrix\Controller\Printer;
use Ipol\Demo\Bitrix\Handler\Order;
use Ipol\Demo\Bitrix\Tools;
use Ipol\Demo\Demo\Handler\BarcodeGeneratorCode128;
use Ipol\Demo\Demo\Handler\BarcodeGeneratorEAN;
...
class BarcodeHandler extends AbstractGeneral
{
    ...
    public static function getStickerRequest()
    {
        if (!(isset($_REQUEST['bitrixId']) && $_REQUEST['bitrixId'])) {
            ...
            $errors = (!empty($result->getData()['ERRORS'])) ? $result->getData()['ERRORS'] : [];
            echo json_encode(array(
                ...
                'errors'  => implode("\n\n", $result->isSuccess() ? $errors : $result->getErrorMessages()),
            ));
        }
    }
    ...
    public static function getStickersRequest($request)
    {
        ...
        if (!isset($request['ids'])) {
            ...
            $dbOrders = OrdersTable::getList(array(
                'select' => ['BITRIX_ID', 'DEMO_GUID', 'DEMO_ID', 'BARK_GENERATE_BY_SERVER'],
                'filter' => ['ID' => $ids]
            ));
            while($tmp = $dbOrders->fetch()) {
                if ($tmp['BARK_GENERATE_BY_SERVER'] === 'Y' && !empty($tmp['DEMO_GUID'])) {
                    $bitrixIds['BY_SERVER'][] = $tmp['BITRIX_ID'];
                } else if ($tmp['BARK_GENERATE_BY_SERVER'] !== 'Y' && !empty($tmp['DEMO_ID'])) {
                    $bitrixIds['BY_MODULE'][] = $tmp['BITRIX_ID'];
                    $files[] = Tools::getJSPath()."ajax.php?".IPOL_DEMO_LBL."action=printBKsRequest&bitrixId=".$tmp['BITRIX_ID'];
                }
            }
            ...
            $errors = (!empty($resultByServer->getData()['ERRORS'])) ? $resultByServer->getData()['ERRORS'] : [];
            echo json_encode(array(
                'success' => $success,
                'files'   => ($success ? $files : []),
                /* At least one generated sticker = success, no matter how much not generated. ERRORS can answer about them */
                'errors'  => is_null($resultByServer) ? '' : implode("\n\n", $resultByServer->isSuccess() ? $errors : $resultByServer->getErrorMessages()),
            ));
        }
    }
    ...
    public static function getStickers($bitrixId)
    {
        ...
        $guids = [];
        $dbOrders = OrdersTable::getList(array(
            'select' => ['BITRIX_ID', 'DEMO_GUID'],
            'filter' => ['BITRIX_ID' => $bitrixId, '!DEMO_GUID' => false, '=BARK_GENERATE_BY_SERVER' => 'Y']
            ));
        while($tmp = $dbOrders->fetch()) {
            $guids[] = $tmp['DEMO_GUID'];
        }
        ...
        if (!empty($guids)) {
            ...
            $result = new Result();
            $result->addError(new Error('Given orders has no demo UUID or not marked for server-side barcode generation.'));
        }
    ...
    public static function getBK($arOrders)
    {
        ...
        $arResult = array(
            ...
            'orders'      => array(),
            'logodemo'   => Tools::getImagePath().'demologo.png',
            'logoCompany' => false
        );
