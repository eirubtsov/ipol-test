...
CModule::IncludeModule('ipol.demo');
CModule::IncludeModule('sale');
CJSCore::Init(array("jquery"));

$module_id = Ipol\Demo\AbstractGeneral::getMODULEID();
$LABEL     = Ipol\Demo\AbstractGeneral::getMODULELBL();

$arAllOptions = Ipol\Demo\Option::toOptions();

$authorized = \Ipol\Demo\AuthHandler::isAuthorized();
...
if ($REQUEST_METHOD == "POST" && strlen($Update.$Apply.$RestoreDefaults) > 0 && check_bitrix_sessid()) {
    if (strlen($RestoreDefaults) > 0) {
        ...
        \Ipol\Demo\OptionsHandler::clearCache(true); // Always clear cache

        $arErrors = array();

        foreach($arAllOptions as $aOptGroup){
            foreach($aOptGroup as $option){
                $validate = \Ipol\Demo\Option::validate($option[0], $_REQUEST[$option[0]]);

                if($validate->isSuccess()){
                    if(\Ipol\Demo\Option::checkMultiple($option[0]))
                    ...
                    if(in_array('image',$option[3])){
                        ...
                        if ($old_fid = \Ipol\Demo\Option::get($option[0]));
                        ...
                        if (strlen($checkRes) <= 0)
                        {
                            ...
                            if ($arPICTURE["del"] == "Y" || strlen($_FILES[$option[0]]["name"]) > 0){
                                \Ipol\Demo\Option::set($option[0],intval($fid));
                            }
                        }
                    } else {
...
if($authorized){
    ...
    if(\Ipol\Demo\Option::get('debug') === 'Y'){
        $arTabs []= array(
            "DIV" => "edit4",
...
function ShowParamsHTMLByArray($arParams,$isHidden = false)
{
    ...
    foreach($arParams as $Option){
        switch ($Option[3][0]){
            case 'selectbox':
                $optVal     = Ipol\Demo\Option::get($Option[0]);
                $selectVals = Ipol\Demo\Option::getSelectVals($Option[0]);
                $attrs      = '';
                $solo       = false;
            ...
            break;
            case 'textbox' :
                Tools::placeOptionRow($Option[1],"<textarea name='".$Option['0']."' id='".$Option['0']."'>".Ipol\Demo\Option::get($Option['0'])."</textarea>");
            break;
            case 'sign' :
                Tools::placeOptionRow($Option[1],Ipol\Demo\Option::get($Option['0'])."<input name='".$Option['0']."' type='hidden' value='".Ipol\Demo\Option::get($Option['0'])."' id='".$Option['0']."'/>");
            break;
            case 'image' :
                $optVal     = Ipol\Demo\Option::get($Option[0]);
...
<script>
    var <?=$LABEL?>setups = new idemo_adminInterface({
...
<?php
    if($authorized) {
        ...
        $isSyncCompleted = (\Ipol\Demo\Option::get('sync_data_completed') == 'Y');
        ...
        if ($isSyncCompleted)
            $buttonPanel .= '<button onclick=\'window.open("/bitrix/admin/ipol_demo_orders.php?lang='.LANGUAGE_ID.'");\' class="ui-btn ui-btn-success">'.Tools::getMessage('LBL_toOrders').'</button>';

        $btnSyncClass = $isSyncCompleted ? 'ui-btn-secondary' : 'ui-btn-danger';
        $buttonPanel .= '<button onclick=\'window.open("/bitrix/admin/ipol_demo_sync_data.php?lang='.LANGUAGE_ID.'");\' class="ui-btn '.$btnSyncClass.'">'.Tools::getMessage('LBL_toSunc').'</button>';
        $buttonPanel .= '</div>';
        ...
        if (!\Ipol\Demo\Warhouses::getWHInfo()) {
            ?><table><?php
            Tools::placeErrorLabel(Tools::getMessage('ERROR_NOWARHOUSES_DESCR'), Tools::getMessage('ERROR_NOWARHOUSES_TITLE'));
            ?></table><?php
        }

        $checkEmptyDels = Ipol\Demo\Bitrix\Handler\Deliveries::getProfilesWithUnconfiguredRateType(false);
...
?>
<form method="post" enctype="multipart/form-data" action="<?=$APPLICATION->GetCurPage()?>?mid=<?=htmlspecialchars($mid)?>&amp;lang=<?=LANG?>">
<?php
    $tabControl->Begin();
    ...
    if($authorized) {?>
        <div align="left">
            ...
            <input type="submit" <?php if(!$USER->IsAdmin())echo " disabled ";?> name="Update" value="<?=GetMessage("MAIN_SAVE")?>">
        </div>
    <?php }?>
    <?php $tabControl->End();?>
