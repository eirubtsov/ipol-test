<?php
use \Ipol\Demo\Admin\OrdersGrid;
...
//if ($GLOBALS['APPLICATION']->GetGroupRight(IPOL_DEMO) > 'D')
...
if (!CheckVersion(SM_VERSION, '17.0.0')) {
    ...
    <script type="text/javascript">
        var <?=IPOL_DEMO_LBL?>controller = new idemo_adminInterface({
            'ajaxPath': '<?=Tools::getJSPath()?>ajax.php',
            'label':    '<?=IPOL_DEMO?>',
            'logging':  true
        });

        <?=IPOL_DEMO_LBL?>controller.expander({});
        <?=IPOL_DEMO_LBL?>controller.addPage('main', {
            init: function(){
                this.actions(this);
                this.grids(this);
            },
            actions: (function(self){
                self.actions = {
                    ...
                    suncStatuses: function(btnLink){
                        ...
                        self.self.ajax({
                            data: {<?=IPOL_DEMO_LBL?>action: 'refreshStatusesAjax'},
                    ...
                    suncOrderStatus: function(bitrixId){
                        self.self.ajax({
                            data: {<?=IPOL_DEMO_LBL?>action: 'checkStatusByBitrixIAjax', bitrixId: bitrixId},
                    ...
                    getSticker: function(bitrixId, barcGenerateByServer){
                        if (barcGenerateByServer === true) {
                            self.self.ajax({
                                data:     {<?=IPOL_DEMO_LBL?>action: 'getStickerRequest', bitrixId: bitrixId},
                                dataType: 'json',
                                success:  self.actions.onOrderSticker
                            });
                        } else {
                            window.open("<?=Tools::getJSPath()."ajax.php?".IPOL_DEMO_LBL."action=printBKsRequest&bitrixId="?>" + bitrixId);
                        }
                    },
                    ...
                    cancelOrder: function(bitrixId){
                        if (confirm('<?=Tools::getMessage('MESS_DOCANCEL')?>')) {
                            self.self.ajax({
                                data:     {<?=IPOL_DEMO_LBL?>action: 'deleteOrder', bitrixId: bitrixId},
                                dataType: 'json',
                    ...
                    eraseOrder: function(bitrixId){
                        if (confirm('<?=Tools::getMessage('TABLE_ORDER_MESS_ERASE_ORDER')?>')) {
                            self.self.ajax({
                                data:     {<?=IPOL_DEMO_LBL?>action: 'eraseOrderAjaxBid', bitrixId: bitrixId},
                                dataType: 'json',
                    ...
                    printStickers: function(ids){
                        self.self.ajax({
                            data     : {<?=IPOL_DEMO_LBL?>action: 'getStickersRequest', ids: ids},
                            dataType : 'json',
                            success  : self.actions.onOrderSticker
                        });
                    },
        ...
        $(document).ready(<?=IPOL_DEMO_LBL?>controller.init);
    </script>
    <?php
}
