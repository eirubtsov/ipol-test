...
#        Site: https://www.ipol.ru
#        E-mail: info@ipol.ru
#        Copyright (c) 2006-2023 IPOL
#################################################

IncludeModuleLangFile(__FILE__);

if (class_exists("ipol_demo"))
    return;

class ipol_demo extends CModule
{
    var $MODULE_ID = "ipol.demo";
    var $MODULE_NAME;
    var $MODULE_VERSION;
    ...
    function __construct()
    {
        ...
        $this->MODULE_NAME = GetMessage("IPOL_DEMO_INSTALL_NAME");
        $this->MODULE_DESCRIPTION = GetMessage("IPOL_DEMO_INSTALL_DESCRIPTION");
    ...
    {
        return array(
                'ipol_demo_orders'    => 'Orders',
                'ipol_demo_points'    => 'Points',
                'ipol_demo_rates'     => 'Rates',
                'ipol_demo_locations' => 'Locations'
        );
    }
    ...
    function DoInstall()
    {
        ...
        if (!CheckVersion($this->getSaleVersion(), '16.0.0')) {
            $GLOBALS['IPOL_DEMO_LBL_INSTALL_ERROR'] = GetMessage('IPOL_DEMO_BADSALEVERSION')." ".$this->getSaleVersion().".";
        }

        if (!function_exists('curl_init')) {
            $GLOBALS['IPOL_DEMO_LBL_INSTALL_ERROR'] = GetMessage('IPOL_DEMO_NOCURL')." ".$this->getSaleVersion().".";
        }

        if ($GLOBALS['IPOL_DEMO_LBL_INSTALL_ERROR']) {
            $GLOBALS['APPLICATION']->IncludeAdminFile(GetMessage('IPOL_DEMO_INSTALL_ERROR_TITLE'), __DIR__ .'/error.php');
            return;
        }
        ...
        $APPLICATION->IncludeAdminFile(GetMessage("IPOL_DEMO_INSTALL"), $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/".$this->MODULE_ID."/install/step1.php");
    }
    ...
    function DoUninstall()
    {
        ...
        if ($_REQUEST['step'] < 2) {
            ...
            if (!array_key_exists('IPOL_DEMO_savedata', $_REQUEST) || $_REQUEST['IPOL_DEMO_savedata'] != 'Y') {
                $this->UnInstallDB();
            }

            $this->UnInstallFiles();

            \Ipol\Demo\AuthHandler::delogin();

            UnRegisterModule($this->MODULE_ID);
            $APPLICATION->IncludeAdminFile(GetMessage("IPOL_DEMO_DEL"), $_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/".$this->MODULE_ID."/install/unstep1.php");
        }
    }

    /**
     * Show save DB tables block while uninstall
     */
    private function ShowDataSaveForm()
    {
        ...
        $APPLICATION->SetTitle(GetMessage('IPOL_DEMO_DEL'));
        ...
        <form action="<?=$APPLICATION->GetCurPage()?>" method="get">
            ...
            <?php CAdminMessage::ShowMessage(GetMessage('IPOL_DEMO_PRESERVE_TABLES'));?>
            <p><?=GetMessage('MOD_UNINST_SAVE')?></p>
            <p><input type="checkbox" name="IPOL_DEMO_savedata" id="IPOL_DEMO_savedata" value="Y" checked="checked" /><label for="savedata"><?=GetMessage('MOD_UNINST_SAVE_TABLES')?></label><br></p>
