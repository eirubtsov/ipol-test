...
function ipol_demo_pvzWidjet(city,deliveries,paysystems,savingInput,postField,pvzPicker,buttonLabel,iPE,LANG,cargo,noYmaps,apikey,yMapsSearch,yMapsSearchMark,showListPvz,defPaysys){
    var self     = this;
    var label    = 'demo pvzWidjet';
    var pvz      = false;
    var paysys   = false;
	var ready    = false;
    ...
    var labelController = {
            ...
            for(var i in deliveries){
                ...
                if(deliveries[i].self) {
                    ...
                    if (typeof(ajaxAns.order) !== 'undefined')
                        deliveries[i].chosen = deliveryHandler.check(deliveries[i].link, ajaxAns.order.DELIVERY);
                    else if (!ajaxAns)
                        deliveries[i].chosen = deliveryHandler.guess(deliveries[i].link);
                }else{
                    if(oldTemplate){
        ...
        place: function(deliveryId){
			...
			if(!ready){
				tmpHTML = '<img src="/bitrix/images/ipol.demo/long_ajax.gif" id="'+iPE+'buttonLoader"><div style="display:none" id="'+iPE+'subLair">' + tmpHTML + "</div>";
			}
    ...
    var addressPropertyController = {
        ...
        checkCorrespond : function(){
            for(var i in deliveries) {
                if (
                    ...
                    if (!deliveries[i].self)
                        return true;
                    else if (deliveries[i].self && deliveries[i].chosen)
                        return true;
                }
            }
        ...
        label : function(){
            ...
            if(input){
                if(pvz){
                    var sub = (typeof(pvz.ADDITIONAL) === 'undefined') ? '' : (' (' + pvz.ADDITIONAL + ') ');
    ...
    // deals with widjet
    var widjetController = new IPOL_DEMO_Widjet({
		popup: true,
		defaultCity : '0c5b2444-70a0-4932-980c-b4dc0d3f02b5',
		path        : '/bitrix/js/ipol.demo/widjet/scripts/',
		servicepath : '/bitrix/js/ipol.demo/ajax.php',
        noYmaps         : noYmaps,
        apikey          : apikey,
        yMapsSearch     : yMapsSearch,
        yMapsSearchMark : yMapsSearchMark,
        showListPVZ     : showListPvz,
		noCitySelector  : true,
		onReady : function(){
			checkPreloader();
			ready = true;
